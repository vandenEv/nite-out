...
const ProfileScreen = ({ navigation }) => {
  ...
  useEffect(() => {
    const fetchUserInfo = async () => {
      try {
        ...
        setUserInfo({ ...userData, userIdToDisplay, profile: userProfile });
      } catch (error) {
  ...
  const userProfileXml = profileIcons[userInfo?.profile] || profileIcons["01"]; // Fallback to "01" if profile not found
    ...
    <View style={styles.container}>
      ...
      <View style={styles.profileContainer}>
        ...
        <TouchableOpacity
          ...
          onPress={() => navigation.navigate("pfpChoice", { gamerId })}
        >
      ...
      <View style={styles.footerContainer}>
        <Text style={styles.forgotPassText}>Need to reset your password? </Text>
