...
const TimeRangeSlider = ({
  ...
  const { sortedIndices, reverseMapping } = useMemo(() => {
    // Map each slot to its original index and sort by start time
    const mappedSlots = timeSlots
      .map((slot, index) => ({
        slot,
        originalIndex: index,
      }))
  ...
  const updateMaxPlayers = (start, end) => {
    ...
    setNumPlayers((currentNumPlayers) =>
      Math.min(currentNumPlayers, minCapacity),
    );
  };
  ...
  const maxCapacity = Math.min(
    ...selectedSlotIndices.map((index) => timeSlots[index].capacity),
  );
  ...
  return (
    <View style={styles.container}>
      ...
      <Text style={styles.label}>
        {getDisplayStartTime()} - {getDisplayEndTime()}
      </Text>
      ...
      {/* Display Max Players */}
      <Text style={styles.maxPlayers}>Max Players: {maxCapacity}</Text>
    </View>
  );
};
